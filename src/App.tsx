...
export default function App() {
  ...
  // Project tags a note can be routed on (see Todos view below). Paths from
  ...
  // "load AND SET all config state" half of what reload() used to do
  // inline, now a named step so reload() itself reads as read+parse inbox,
  // load+apply config, then auto-tag.
  // Note: calls setZoom, which useZoom (below) doesn't declare until after
  // this point — safe only because applyConfig is invoked from async
  // reload(), never synchronously during this render; a synchronous
  // render-time call here would hit the TDZ.
